...
- **Import tab** — a drop zone takes CSV, OFX-family (`.ofx`/`.qfx`/`.qbo`), image, and PDF files; they're written to `.capy/import/sources/` on drop. A **Start** button becomes active once files are present. A manual drop writes only `sources/` — never `state.json` — so the screen waits for Start.
- **Chat** — when a file arrives in a chat turn, Capy calls the `start_import` tool, which copies the turn's attachments into `.capy/import/sources/`, writes a `state.json` marked `source: "chat"`, and replies pointing the user at the Import tab. The Import screen, seeing sources with no `transactions.csv` and a `state.json` it didn't write, auto-runs the orchestrator. The bytes live on the chat turn (text inlined, images base64); the handler stages from the turn's attachments, never from anything the model echoes into its arguments. When staging is already occupied — a run in flight or parked for review, or files dropped in the Import tab but not started — the tool refuses with guidance pointing at the Import tab instead of replacing that import. A re-share replaces staging only when there's nothing to protect: a prior chat staging that never ran, or a completed empty preview (staged rows exist but are zero-length — nothing was importable). A non-empty staged preview refuses.
...
- **Structured (CSV):** code first locates the table's real header — a consistency scan over the parsed rows picks the row whose field count the following rows sustain (bank exports often prepend a summary preamble), and blank or duplicate header cells are renamed positionally (`Column 2`) so every column stays addressable. The model returns a `CsvMapping` (date column + format, description column(s), how amounts are structured — single signed column or split debit/credit — and formatted, how to detect expense/income/transfer, the source account, the source category column, skip rules for non-transaction rows, and an optional `headerRow` overriding the detected header when code picked wrong). The prompt carries the user's existing account names, so a literal source account comes back as an existing account's exact name when the file clearly matches one. Code applies it to every row. If a code-side preview of the first rows surfaces transform errors, the model gets one correction round with the errors attached; whatever the second mapping produces is final.
- **Unstructured (receipts, bank screenshots, statement scans):** the model reads the image (image bytes as base64 `image` content, PDF bytes as base64 `document` content) and emits the same intermediate records directly — `{ date, amount, type, description, sourceAccount, sourceCategory }`. The merchant it reads becomes `description` (the match key); a category it infers becomes `sourceCategory` (where a CSV's category column lands). The prompt names today's date for rows without a parseable one (pending transactions, a cropped date column), and carries the source filename plus the user's existing account names — `sourceAccount` comes back as a matching account's exact name when the statement clearly belongs to one, else a short descriptive or filename-derived name, never empty. A bank screenshot comes out indistinguishable from a transformed CSV. The extraction call **streams**: the model declares its transaction `count` first (the schema orders it before `rows`), then the rows, so the Normalizing meter fills against a real denominator while the long extraction runs — extraction is the phase's long pole and would otherwise sit visually frozen.
- **Deterministic (OFX family — `.ofx`/`.qfx`/`.qbo`):** OFX-family exports carry standardized fields, so there is **no model round-trip** — the deterministic sibling of the CSV mapper. Code parses the SGML (OFX 1.x) or XML (OFX 2.x) tree and maps each transaction straight to the intermediate record: `DTPOSTED` → date (date part only; the `[zone]` suffix is dropped), `TRNAMT` → signed-cents `amount` (OFX already uses capy's negative-outflow convention — a card purchase is negative), `NAME` (plus `MEMO` when distinct) → `description`, and the statement's institution name → `sourceAccount` (which grounding then matches against an existing account by name). Type is read from the amount sign, except an `XFER`, or a credit-card `PAYMENT` — a card payoff — which is a transfer. Both the credit-card (`CCSTMTTRNRS`/`CCSTMTRS`) and bank (`STMTTRNRS`/`STMTRS`) shapes are handled. Since the fields are exact, this path is cheaper and more accurate than routing the same file through the CSV mapper. An empty or unparseable OFX yields no rows — handled like any other no-data file (see **Errors**).

The resolved `merchant` / `categoryId` fields stay **empty** in every path — those are History's and Categorizing's job. So there are no pre-filled candidates to reconcile: Capy's receipt reads ride the raw `description` + `sourceCategory` channels and get resolved by History exactly like CSV signal.

Both paths feed one builder — `buildStaged(records) → ImportTransaction[]` — that owns id assignment (sequential `imp-N`, continuing across files), the trim-45 `description`, sign/type normalization, date coercion (any supported format → `YYYY-MM-DD`; an unparseable date — a "Pending" column, a cropped screenshot — falls back to today, since staging read-back drops non-ISO dates), and empty resolved fields. An empty `sourceAccount` surviving either path (a blank account cell, a model empty string) heals to a filename-derived name ("Imported" when the filename yields nothing) — no staged row is ever account-less. One staging format, one set of invariants.
...
- **No transaction data** (a selfie, a non-financial document): the extraction call returns a structured `{ error: "no_data" }` (an empty `{ rows: [] }` is treated the same). When several files are imported together, a no-data file is skipped with a warning and the run continues on the rest. When *every* file is empty, the run stages an empty set and completes on the preview's empty state — not an error — and Cancel clears it.
...
- **Account mapping** lives in a set of mapping rows — one per imported account, each showing the imported name → a target-account selector (an existing budget account, or create-on-merge — the default), edits applying live. The same rows render in two places: the Map-accounts dialog the table's ACCOUNT cells open (every imported account), and the merge confirmation (only the source accounts with selected rows, so the gate reflects exactly what merges). The ACCOUNT cell shows only the mapped *target* name, with a "new" tag when merge would create the account — the raw imported → target relationship is visible in the rows. The merge dialog is an editable safety gate: titled with the transaction count ("Merge N transactions?") and subtitled with the total it adds ("This will add **{total}** to your budget"), it lists those same source → target rows so a wrong mapping is both visible and correctable before commit — fixing it there flows into the merge. Rows are the account-mapping axis only; a transfer's counterpart is not a mapping and gets no row. **Transfers that lose their counterpart** — the merge couldn't pair them, so they import as income/expense — collapse into a single amber warning line with their count, never enumerated. The confirmation's rows scroll within the dialog body (the selector's dropdown portals out so it isn't clipped); the Map-accounts dialog caps the list at five rows. Either way the Cancel/Merge buttons stay pinned on-screen. Aliases stored in `.capy/aliases.json` survive across imports and pre-populate the mapping; they're persisted at merge only for the sources that actually merged a row, so a fully-deselected source leaks no mapping into the next import.
